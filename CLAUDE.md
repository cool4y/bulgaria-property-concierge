...
- **Positioning keywords** (homepage title, descriptions, FAQ, services line, properties page, blog): the full-cycle "A to Z" service is the differentiator and leads the title; luxury / boutique / penthouse / premium segment / high-end; and foreigners / foreign investors. Listing titles carry one truthful segment word each (luxury, boutique, premium, high-end). The Properties page and homepage listings use the same type sizes as the blog (`#projectGrid`, `#propGrid` block at the end of `css/brand.css`; intro reuses `.blog-intro` / `.blog-lead`).
- **Do not change visible text on the homepage for SEO.** Keywords go into hidden tags (title, meta, og, keywords, JSON-LD), the blog, the Properties page and Q&A only; the homepage hero and section copy stay as the client wrote them. (The Black Sea Villa card on the homepage has its own `-home` i18n keys so the Properties page can say "luxury" without changing the homepage.)

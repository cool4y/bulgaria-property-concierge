/* Bulgarian translations. Loaded by js/i18n.js on every page.
 *
 * Format:  "key": "Bulgarian text",   (the `// EN:` line above each entry is the English original)
 * - Keys are attached to the HTML with data-i18n="key". Empty "" means "not translated yet"
 *   and falls back to English.
 * - Entries whose English contains markup (<span>, <br/>, ...) are HTML: keep the same tags.
 * - After adding or changing English text in a page run:  python tools/i18n_tool.py tag
 *   then translate the empty entries here, and finish with:  python tools/i18n_tool.py check
 */
window.I18N_BG = {
  // EN: Bulgaria Property Concierge | Property Investment & Homes in Sofia, Bulgaria
  "meta-title-home": "Bulgaria Property Concierge | Инвестиции в имоти и домове в София, България",

  // EN: Sofia-based full-cycle property service for local and international buyers: sourcing, legal, renovation, furnishing and rental management under one roof. Book a free consultation.
  "meta-description-home": "Цялостна услуга за имоти със седалище в София за местни и международни купувачи: търсене, правни услуги, ремонт, обзавеждане и управление на наеми под един покрив. Запазете безплатна консултация.",

  // EN: Language
  "language": "Език",

  // EN: Bulgaria's only full-cycle property service.
  "hero-headline": "Единствената в България услуга за имоти от А до Я.",

  // EN: Properties in Bulgaria | Completed Projects & Listings | Bulgaria Property Concierge
  "meta-title-properties": "Имоти в България | Завършени проекти и обяви | Bulgaria Property Concierge",

  // EN: See our completed projects with real results, and browse move-in ready and off-plan properties for sale in Sofia, Varna, Plovdiv, Bansko and Sozopol.
  "meta-description-properties": "Вижте нашите завършени проекти с реални резултати и разгледайте имоти за продажба - готови за нанасяне и на зелено - в София, Варна, Пловдив, Банско и Созопол.",

  // EN: Main navigation
  "main-navigation": "Основна навигация",

  // EN: Process
  "process": "Процес",

  // EN: Services
  "services": "Услуги",

  // EN: Properties
  "properties": "Имоти",

  // EN: Why Us
  "why-us": "Защо ние",

  // EN: Why Bulgaria
  "why-bulgaria": "Защо България",

  // EN: FAQ
  "faq": "Въпроси",

  // EN: Journal
  "journal": "Блог",

  // EN: Book Consultation
  "book-consultation": "Запазете час",

  // EN: Open menu
  "open-menu": "Отвори менюто",

  // EN: Mobile navigation
  "mobile-navigation": "Мобилна навигация",

  // EN: Alexander Nevsky Cathedral in Sofia at dusk with Vitosha mountain behind the city
  "alexander-nevsky-cathedral-in-sofia-at-dusk": "Катедралата „Александър Невски“ в София по здрач, с Витоша зад града",

  // EN: One partner. From sourcing property to keys.
  "one-partner-from-sourcing-property-to": "Един партньор. От търсенето на имот до ключовете.",

  // EN: Sofia.
  "sofia": "София.",

  // EN: Whether you're buying from abroad or here at home, we manage every step: from sourcing the property to a fully finished home. <span class="text-navy font-medium">Completely hassle-free.</span>
  "whether-you-re-buying-from-abroad": "Независимо дали купувате от чужбина, или тук, у дома, ние поемаме всяка стъпка: от намирането на имота до напълно завършен дом. <span class=\"text-navy font-medium\">Без никакви грижи.</span>",

  // EN: Book a Free Consultation
  "book-a-free-consultation": "Безплатна консултация",

  // EN: Chat on Viber
  "chat-on-viber": "Пишете във Viber",

  // EN: Chat on WhatsApp
  "chat-on-whatsapp": "Пишете в WhatsApp",

  // EN: Years<br/>Experience
  "years-experience": "години<br/>опит",

  // EN: Scroll
  "scroll": "Надолу",

  // EN: How It Works
  "how-it-works": "Как работим",

  // EN: A simple six-step process.
  "a-simple-six-step-process": "Прост процес в шест стъпки.",

  // EN: Strategy Call
  "strategy-call": "Стратегически разговор",

  // EN: We learn your goals, budget, and timeline.
  "we-learn-your-goals-budget-and": "Уточняваме целите, бюджета и сроковете ви.",

  // EN: Property Search
  "property-search": "Търсене на имот",

  // EN: A curated shortlist matched to your brief.
  "a-curated-shortlist-matched-to-your": "Подбрана селекция според вашите изисквания.",

  // EN: Legal Due Diligence
  "legal-due-diligence": "Правна проверка",

  // EN: Lawyers verify title, permits, encumbrances.
  "lawyers-verify-title-permits-encumbrances": "Адвокати проверяват собствеността, разрешителните и тежестите.",

  // EN: Purchase
  "purchase": "Покупка",

  // EN: Smooth notary process, full legal support.
  "smooth-notary-process-full-legal-support": "Гладък нотариален процес и пълна правна подкрепа.",

  // EN: Renovation & Furnishing
  "renovation-and-furnishing": "Ремонт и обзавеждане",

  // EN: Design, build, furnish - turnkey, on budget.
  "design-build-furnish-turnkey-on-budget": "Дизайн, изпълнение, обзавеждане - под ключ и в рамките на бюджета.",

  // EN: Rental & Management
  "rental-and-management": "Наем и управление",

  // EN: We find tenants and manage long-term.
  "we-find-tenants-and-manage-long": "Намираме наематели и управляваме дългосрочно.",

  // EN: Full-Cycle Service
  "full-cycle-service": "Цялостна услуга",

  // EN: From your dream property to a finished home.
  "from-your-dream-property-to-a": "От мечтания имот до готов дом.",

  // EN: Sourcing, purchase, financing, design and delivery - one dedicated team manages every step, so you never coordinate contractors, lawyers or agencies from abroad.
  "sourcing-purchase-financing-design-and-delivery": "Търсене, покупка, финансиране, дизайн и предаване - един отдаден екип управлява всяка стъпка, така че никога не се налага да координирате изпълнители, адвокати или агенции от чужбина.",

  // EN: Residential Property
  "residential-property": "Жилищни имоти",

  // EN: Curated homes and apartments matched to your lifestyle and budget.
  "curated-homes-and-apartments-matched-to": "Внимателно подбрани домове и апартаменти според вашия начин на живот и бюджет.",

  // EN: Commercial Real Estate
  "commercial-real-estate": "Търговски недвижими имоти",

  // EN: Offices, retail, and mixed-use assets aligned to your investment thesis.
  "offices-retail-and-mixed-use-assets": "Офиси, търговски и смесени обекти, съобразени с вашата инвестиционна стратегия.",

  // EN: Investment Consulting
  "investment-consulting": "Инвестиционни консултации",

  // EN: Strategy, market intelligence, and portfolio structuring.
  "strategy-market-intelligence-and-portfolio-structuring": "Стратегия, пазарен анализ и структуриране на портфейл.",

  // EN: Legal Assistance
  "legal-assistance": "Правна помощ",

  // EN: Full legal support: contracts, due diligence, and compliance.
  "full-legal-support-contracts-due-diligence": "Пълна правна подкрепа: договори, правна проверка и съответствие с изискванията.",

  // EN: Financial Consulting
  "financial-consulting": "Финансови консултации",

  // EN: Tax planning, entity setup, and mortgage arrangement.
  "tax-planning-entity-setup-and-mortgage": "Данъчно планиране, регистрация на фирма и уреждане на ипотечен кредит.",

  // EN: Renovation
  "renovation": "Ремонт",

  // EN: Trusted contractors, transparent budgets, on-time delivery.
  "trusted-contractors-transparent-budgets-on-time": "Доверени изпълнители, прозрачни бюджети, спазване на сроковете.",

  // EN: Interior Design
  "interior-design": "Интериорен дизайн",

  // EN: Editorial interiors that elevate rental yield and asset value.
  "editorial-interiors-that-elevate-rental-yield": "Изискани интериори, които повишават наемната доходност и стойността на имота.",

  // EN: Custom Furniture
  "custom-furniture": "Мебели по поръчка",

  // EN: Bespoke kitchens, joinery, and furnishing packages.
  "bespoke-kitchens-joinery-and-furnishing-packages": "Кухни, вградени мебели и пакети за обзавеждане по поръчка.",

  // EN: Rental Management
  "rental-management": "Управление на наеми",

  // EN: Tenant sourcing, contracts, collections, and reporting.
  "tenant-sourcing-contracts-collections-and-reporting": "Намиране на наематели, договори, събиране на наеми и отчетност.",

  // EN: Interested to know more?
  "interested-to-know-more": "Искате да научите повече?",

  // EN: Talk to an advisor
  "talk-to-an-advisor": "Говорете с консултант",

  // EN: Three Execution Tiers
  "three-execution-tiers": "Три нива на изпълнение",

  // EN: Every service, scaled to your budget.
  "every-service-scaled-to-your-budget": "Всяка услуга - според вашия бюджет.",

  // EN: From efficient buy-to-let finishes to fully bespoke luxury execution - the process is the same, only the finish level changes.
  "from-efficient-buy-to-let-finishes": "От ефективно довършване за отдаване под наем до напълно индивидуално луксозно изпълнение - процесът е един и същ, променя се само нивото на довършване.",

  // EN: Essential tier - bedroom and bathroom finish
  "essential-tier-bedroom-and-bathroom-finish": "Основно ниво - довършване на спалня и баня",

  // EN: Essential tier - living room and kitchen finish
  "essential-tier-living-room-and-kitchen": "Основно ниво - довършване на хол и кухня",

  // EN: Previous photo
  "previous-photo": "Предишна снимка",

  // EN: Next photo
  "next-photo": "Следваща снимка",

  // EN: Photo 1
  "photo-1": "Снимка 1",

  // EN: Photo 2
  "photo-2": "Снимка 2",

  // EN: Budget
  "budget": "Икономичен",

  // EN: Essential
  "essential": "Основен",

  // EN: Efficient, quality finishes built for rental yield and fast turnaround.
  "efficient-quality-finishes-built-for-rental": "Ефективно, качествено довършване, съобразено с наемната доходност и бързите срокове.",

  // EN: Design & Planning
  "design-and-planning": "Дизайн и планиране",

  // EN: 2D floor plan & space-layout confirmation
  "2d-floor-plan-and-space-layout": "2D план и потвърждение на разпределението",

  // EN: Pre-selected tile, paint & flooring palette (curated options)
  "pre-selected-tile-paint-and-flooring": "Предварително подбрана палитра от плочки, бои и настилки (подбрани варианти)",

  // EN: Fixtures & Furniture
  "fixtures-and-furniture": "Оборудване и мебели",

  // EN: Standard kitchen, bathroom & lighting fixtures
  "standard-kitchen-bathroom-and-lighting-fixtures": "Стандартно оборудване за кухня, баня и осветление",

  // EN: Flat-pack furniture sourcing & assembly
  "flat-pack-furniture-sourcing-and-assembly": "Доставка и монтаж на мебели в разглобен вид",

  // EN: Execution
  "execution": "Изпълнение",

  // EN: Trusted contractor network, fixed-fee build team
  "trusted-contractor-network-fixed-fee-build": "Мрежа от доверени изпълнители, строителен екип с фиксирано възнаграждение",

  // EN: Fixed-scope pricing, no change-order surprises
  "fixed-scope-pricing-no-change-order": "Фиксирана цена по обхват, без неприятни изненади",

  // EN: Up to 90 days
  "up-to-90-days": "До 90 дни",

  // EN: Request Quote
  "request-quote": "Поискайте оферта",

  // EN: Most Popular
  "most-popular": "Най-предпочитан",

  // EN: Signature tier - living, kitchen, bedroom and bathroom finish
  "signature-tier-living-kitchen-bedroom-and": "Авторско ниво - довършване на хол, кухня, спалня и баня",

  // EN: Mid-range
  "mid-range": "Среден клас",

  // EN: Signature
  "signature": "Авторски",

  // EN: Our most-chosen tier - considered design and materials for long-term value.
  "our-most-chosen-tier-considered-design": "Най-избираното от нашите нива - премислен дизайн и материали за дългосрочна стойност.",

  // EN: Full space-planning & layout optimization
  "full-space-planning-and-layout-optimization": "Цялостно планиране и оптимизация на разпределението",

  // EN: Custom 3D renders of every room before construction starts
  "custom-3d-renders-of-every-room": "3D визуализации по поръчка на всяка стая преди началото на строителството",

  // EN: Materials & Furniture
  "materials-and-furniture": "Материали и мебели",

  // EN: Mid-range tile, flooring & fixture selection from EU/BG suppliers
  "mid-range-tile-flooring-and-fixture": "Плочки, настилки и оборудване от среден клас от доставчици от ЕС и България",

  // EN: Selected furniture with great durability and modern style
  "selected-furniture-with-great-durability-and": "Подбрани мебели с отлична издръжливост и модерен стил",

  // EN: Bespoke kitchen & wardrobe carpentry
  "bespoke-kitchen-and-wardrobe-carpentry": "Кухня и гардероби по поръчка",

  // EN: Full project management, single point of contact
  "full-project-management-single-point-of": "Цялостно управление на проекта, едно лице за контакт",

  // EN: 90–120 days
  "90-120-days": "90–120 дни",

  // EN: Bespoke tier - living and dining space with marble table
  "bespoke-tier-living-and-dining-space": "Индивидуално ниво - хол и трапезария с мраморна маса",

  // EN: Bespoke tier - walk-in dressing room
  "bespoke-tier-walk-in-dressing-room": "Индивидуално ниво - гардеробна",

  // EN: Bespoke tier - primary bedroom with marble feature wall
  "bespoke-tier-primary-bedroom-with-marble": "Индивидуално ниво - главна спалня с мраморна стена",

  // EN: Bespoke tier - marble bathroom with freestanding tub
  "bespoke-tier-marble-bathroom-with-freestanding": "Индивидуално ниво - мраморна баня със свободностояща вана",

  // EN: Photo 3
  "photo-3": "Снимка 3",

  // EN: Photo 4
  "photo-4": "Снимка 4",

  // EN: Luxury
  "luxury": "Луксозен",

  // EN: Bespoke
  "bespoke": "Индивидуален",

  // EN: Fully custom architecture, materials and furnishing - no compromises.
  "fully-custom-architecture-materials-and-furnishing": "Изцяло индивидуална архитектура, материали и обзавеждане - без компромиси.",

  // EN: Bespoke architecture & space planning from scratch
  "bespoke-architecture-and-space-planning-from": "Индивидуална архитектура и планиране на пространството от нулата",

  // EN: Full 3D visualization & walkthrough before build begins
  "full-3d-visualization-and-walkthrough-before": "Пълна 3D визуализация и виртуална разходка преди началото на строежа",

  // EN: Custom-designed tiles, mosaics & decorative wall finishes
  "custom-designed-tiles-mosaics-and-decorative": "Плочки, мозайки и декоративни стенни покрития по индивидуален дизайн",

  // EN: Commissioned, bespoke furniture design
  "commissioned-bespoke-furniture-design": "Мебели по индивидуален дизайн, изработени по поръчка",

  // EN: Unrestricted material sourcing - Italian marble, natural stone, imported hardwood
  "unrestricted-material-sourcing-italian-marble-natural": "Без ограничения при избора на материали - италиански мрамор, естествен камък, внесена дървесина",

  // EN: High-end finishes - designer lighting, smart-home integration, imported hardware
  "high-end-finishes-designer-lighting-smart": "Премиум покрития - дизайнерско осветление, интеграция на умен дом, внесена фурнитура",

  // EN: 90–150 days
  "90-150-days": "90–150 дни",

  // EN: Each project is scoped individually - timelines may vary based on scope and site conditions.
  "each-project-is-scoped-individually-timelines": "Всеки проект се оценява индивидуално - сроковете могат да варират според обхвата и състоянието на обекта.",

  // EN: By Invitation
  "by-invitation": "Само с покана",

  // EN: Come see it before you decide.
  "come-see-it-before-you-decide": "Елате и вижте, преди да решите.",

  // EN: A private, three-day discovery tour of Sofia's best off-market and exclusive listings - curated to your brief, guided by our team from the moment you land.
  "a-private-three-day-discovery-tour": "Частна тридневна обиколка на най-добрите ексклузивни оферти извън публичния пазар в София - съобразена с вашите изисквания и с наш екип до вас от момента на кацането.",

  // EN: Fully credited toward<br/>your purchase
  "fully-credited-toward-your-purchase": "Приспада се изцяло<br/>от цената на покупката",

  // EN: Request Your VIP Tour
  "request-your-vip-tour": "Заявете VIP тур",

  // EN: Arrival & Welcome
  "arrival-and-welcome": "Пристигане и посрещане",

  // EN: Private airport transfer and an orientation briefing tailored to your goals.
  "private-airport-transfer-and-an-orientation": "Личен трансфер от летището и ориентационна среща според вашите цели.",

  // EN: Private Property Tours
  "private-property-tours": "Частни огледи на имоти",

  // EN: Two full days viewing curated, exclusive and off-market opportunities.
  "two-full-days-viewing-curated-exclusive": "Два пълни дни огледи на подбрани, ексклузивни оферти извън публичния пазар.",

  // EN: Decide With Confidence
  "decide-with-confidence": "Решавайте уверено",

  // EN: Leave with a shortlist, real numbers, and a clear next step - no pressure.
  "leave-with-a-shortlist-real-numbers": "Заминавате с кратък списък, реални цифри и ясна следваща стъпка - без натиск.",

  // EN: Listings
  "listings": "Обяви",

  // EN: Exclusive access. Flawless execution.
  "exclusive-access-flawless-execution": "Ексклузивен достъп. Безупречно изпълнение.",

  // EN: Investment, residential, or vacation property - every project is sourced through our private network and finished to the same exacting standard.
  "investment-residential-or-vacation-property-every": "Инвестиционен, жилищен или ваканционен имот - всеки проект се намира чрез нашата частна мрежа и се довършва по един и същ висок стандарт.",

  // EN: Renovated Apartment in Ivan Vazov - Sofia · Ivan Vazov · 2026
  "renovated-apartment-in-ivan-vazov-sofia": "Ремонтиран апартамент в Иван Вазов - София · Иван Вазов · 2026",

  // EN: Sofia · Ivan Vazov · 2026
  "sofia-ivan-vazov-2026": "София · Иван Вазов · 2026",

  // EN: Renovated Apartment in Ivan Vazov
  "renovated-apartment-in-ivan-vazov": "Ремонтиран апартамент в Иван Вазов",

  // EN: A fully renovated one-bedroom apartment in one of Sofia's most walkable central districts, let within weeks of handover.
  "a-fully-renovated-one-bedroom-apartment": "Изцяло ремонтиран апартамент с една спалня в един от най-удобните за разходка централни райони на София, отдаден под наем броени седмици след предаването.",

  // EN: Timeline
  "timeline": "Срок",

  // EN: 2.5 mo
  "2-5-mo": "2,5 мес.",

  // EN: Yield
  "yield": "Доходност",

  // EN: Furnished Duplex in Lozenets - Sofia · Lozenets · 2026
  "furnished-duplex-in-lozenets-sofia-lozenets": "Обзаведен дуплекс в Лозенец - София · Лозенец · 2026",

  // EN: Sofia · Lozenets · 2026
  "sofia-lozenets-2026": "София · Лозенец · 2026",

  // EN: Furnished Duplex in Lozenets
  "furnished-duplex-in-lozenets": "Обзаведен дуплекс в Лозенец",

  // EN: A two-bedroom duplex in Sofia's most sought-after residential district, delivered turnkey for a relocating family.
  "a-two-bedroom-duplex-in-sofia": "Дуплекс с две спални в най-търсения жилищен район на София, предаден под ключ на семейство, което се премества.",

  // EN: 4 mo
  "4-mo": "4 мес.",

  // EN: Bedrooms
  "bedrooms": "Спални",

  // EN: Hillside House in Boyana
  "hillside-villa-in-boyana-living-and": "Къща на склона в Бояна",

  // EN: Sofia · Boyana · 2025
  "sofia-boyana-2025": "София · Бояна · 2025",

  // EN: Hillside House in Boyana
  "hillside-villa-in-boyana": "Къща на склона в Бояна",

  // EN: A four-bedroom hillside house at the foot of Vitosha, fully custom-finished for a private client.
  "a-four-bedroom-hillside-villa-at": "Къща с четири спални на склон в подножието на Витоша, изцяло довършена по индивидуален проект за частен клиент.",

  // EN: 5.5 mo
  "5-5-mo": "5,5 мес.",

  // EN: Old Town Family Home - Plovdiv · Old Town · 2023
  "old-town-family-home-plovdiv-old": "Семеен дом в Стария град - Пловдив · Старият град · 2023",

  // EN: Plovdiv · Old Town · 2023
  "plovdiv-old-town-2023": "Пловдив · Старият град · 2023",

  // EN: Old Town Family Home
  "old-town-family-home": "Семеен дом в Стария град",

  // EN: Two-bedroom apartment finished and furnished for a family relocating from abroad.
  "two-bedroom-apartment-finished-and-furnished": "Апартамент с две спални, довършен и обзаведен за семейство, което се премества от чужбина.",

  // EN: Black Sea Villa - Sozopol · Coastal · 2024
  "black-sea-villa-sozopol-coastal-2024": "Вила на Черно море - Созопол · Крайбрежие · 2024",

  // EN: Sozopol · Coastal · 2024
  "sozopol-coastal-2024": "Созопол · Крайбрежие · 2024",

  // EN: Black Sea Villa
  "black-sea-villa": "Вила на Черно море",

  // EN: A four-bedroom seafront villa, restored and furnished as a private family retreat.
  "a-four-bedroom-seafront-villa-restored": "Вила с четири спални на брега на морето, реставрирана и обзаведена като частно семейно убежище.",

  // EN: 7 mo
  "7-mo": "7 мес.",

  // EN: Boutique Office in Lozenets - Sofia · Lozenets · 2023
  "boutique-office-in-lozenets-sofia-lozenets": "Бутиков офис в Лозенец - София · Лозенец · 2023",

  // EN: Sofia · Lozenets · 2023
  "sofia-lozenets-2023": "София · Лозенец · 2023",

  // EN: Boutique Office in Lozenets
  "boutique-office-in-lozenets": "Бутиков офис в Лозенец",

  // EN: A boutique office fit-out, delivered turnkey and leased within a month.
  "a-boutique-office-fit-out-delivered": "Бутиково офис обзавеждане, предадено под ключ и отдадено под наем в рамките на месец.",

  // EN: 3 mo
  "3-mo": "3 мес.",

  // EN: Explore More Properties
  "explore-more-properties": "Вижте още имоти",

  // EN: Client reviewing floor plans on a property viewing, overlooking a Sofia construction site and the surrounding mountains
  "client-reviewing-floor-plans-on-a": "Клиент разглежда планове по време на оглед на имот, с изглед към строителен обект в София и околните планини",

  // EN: Why Choose Us
  "why-choose-us": "Защо да изберете нас",

  // EN: Your trusted partner on the ground.
  "your-trusted-partner-on-the-ground": "Вашият доверен партньор на място.",

  // EN: Real estate experts, project managers, renovation workers, designers, custom furniture makers - one team of seasoned specialists, accountable to you from search to management.
  "real-estate-experts-project-managers-renovation": "Експерти по недвижими имоти, ръководители на проекти, строители, дизайнери, производители на мебели по поръчка - един екип от опитни специалисти, който отговаря пред вас от търсенето до управлението.",

  // EN: Market Expertise
  "market-expertise": "Познаване на пазара",

  // EN: Local market expertise - guiding you to neighborhoods that grow in value.
  "local-market-expertise-guiding-you-to": "Задълбочено познаване на местния пазар - насочваме ви към квартали, чиято стойност расте.",

  // EN: One Point of Contact
  "one-point-of-contact": "Едно лице за контакт",

  // EN: A dedicated advisor who owns your project end-to-end.
  "a-dedicated-advisor-who-owns-your": "Личен консултант, който отговаря за проекта ви от началото до края.",

  // EN: Transparency
  "transparency": "Прозрачност",

  // EN: Every fee, every step - documented and shared with you.
  "every-fee-every-step-documented-and": "Всяка такса, всяка стъпка - документирана и споделена с вас.",

  // EN: Long-Term Partnership
  "long-term-partnership": "Дългосрочно партньорство",

  // EN: We stay with you long after the keys change hands.
  "we-stay-with-you-long-after": "Оставаме до вас дълго след предаването на ключовете.",

  // EN: Years of Real Estate Experience
  "years-of-real-estate-experience": "Години опит в недвижимите имоти",

  // EN: Properties Sold
  "properties-sold": "Продадени имоти",

  // EN: First-Class Delivery & Client Satisfaction
  "first-class-delivery-and-client-satisfaction": "Първокласно изпълнение и удовлетворени клиенти",

  // EN: Working with Bulgaria's best - construction, furniture and banking partners
  "working-with-bulgaria-s-best-construction": "Работим с най-добрите в България - партньори в строителството, мебелите и банкирането",

  // EN: Aleti Furniture
  "aleti-furniture": "Aleti Furniture",

  // EN: DOMEX
  "domex": "DOMEX",

  // EN: Arreda Casa Furniture & Design
  "arreda-casa-furniture-and-design": "Arreda Casa Furniture & Design",

  // EN: UniCredit Bulbank
  "unicredit-bulbank": "UniCredit Bulbank",

  // EN: BETO
  "beto": "BETO",

  // EN: TrioKitchen
  "triokitchen": "TrioKitchen",

  // EN: A rare combination of local access and international professionalism. I now hold two properties through them.
  "a-rare-combination-of-local-access": "Рядка комбинация от местен достъп и международен професионализъм. Вече притежавам два имота чрез тях.",

  // EN: Nikolaj B. - Investor from Denmark
  "nikolaj-b-investor-from-denmark": "Nikolaj B. - инвеститор от Дания",

  // EN: The smart move - from abroad or at home.
  "the-smart-move-from-abroad-or": "Умният ход - от чужбина или от България.",

  // EN: Bulgaria's entry into the Eurozone opens a new era of stability and growth. Attractive prices, low taxes and rising rental demand make Sofia and the wider country one of Europe's most promising property markets - for international buyers and Bulgarians alike.
  "bulgaria-s-entry-into-the-eurozone": "Влизането на България в еврозоната открива нова ера на стабилност и растеж. Атрактивните цени, ниските данъци и нарастващото търсене на наеми правят София и страната като цяло един от най-перспективните имотни пазари в Европа - както за чуждестранни купувачи, така и за българи.",

  // EN: Eurozone Member
  "eurozone-member": "Член на еврозоната",

  // EN: Added stability, currency confidence, and access to the deep European capital market.
  "added-stability-currency-confidence-and-access": "Допълнителна стабилност, доверие във валутата и достъп до дълбокия европейски капиталов пазар.",

  // EN: Growing Economy
  "growing-economy": "Растяща икономика",

  // EN: Steady GDP expansion, rising incomes, and one of the EU's lowest debt-to-GDP ratios.
  "steady-gdp-expansion-rising-incomes-and": "Стабилен ръст на БВП, нарастващи доходи и едно от най-ниските нива на дълг спрямо БВП в ЕС.",

  // EN: Attractive Prices
  "attractive-prices": "Атрактивни цени",

  // EN: Lower entry costs than Western Europe with equivalent build quality in prime areas.
  "lower-entry-costs-than-western-europe": "По-ниски входни разходи в сравнение със Западна Европа при съпоставимо качество на строителството в престижните райони.",

  // EN: Low Taxes
  "low-taxes": "Ниски данъци",

  // EN: 10% flat corporate and personal income tax - among the lowest in the European Union.
  "10-flat-corporate-and-personal-income": "10% плосък корпоративен и личен данък върху доходите - сред най-ниските в Европейския съюз.",

  // EN: Rental Demand
  "rental-demand": "Търсене на наеми",

  // EN: Gross yields of 5–8% in major cities and 8–12% in coastal and ski markets.
  "gross-yields-of-5-8-in": "Брутна доходност от 5–8% в големите градове и 8–12% на морските и ски пазарите.",

  // EN: Strategic Location
  "strategic-location": "Стратегическо разположение",

  // EN: Bridging the EU, the Balkans, the Black Sea and Asia - with fast, affordable flights.
  "bridging-the-eu-the-balkans-the": "Мост между ЕС, Балканите, Черно море и Азия - с бързи и достъпни полети.",

  // EN: Frequently Asked
  "frequently-asked": "Често задавани",

  // EN: Answers, before you ask.
  "answers-before-you-ask": "Отговори, преди да попитате.",

  // EN: Still have a question?
  "still-have-a-question": "Все още имате въпрос?",

  // EN: Ask us directly
  "ask-us-directly": "Питайте ни директно",

  // EN: Can foreigners buy property in Bulgaria?
  "can-foreigners-buy-property-in-bulgaria": "Могат ли чужденци да купуват имоти в България?",

  // EN: Yes. EU citizens buy directly. Non-EU citizens can buy apartments and buildings freely, and land via a Bulgarian company we can set up for you.
  "yes-eu-citizens-buy-directly-non": "Да. Гражданите на ЕС купуват директно. Гражданите извън ЕС могат свободно да купуват апартаменти и сгради, а земя - чрез българска фирма, която можем да регистрираме за вас.",

  // EN: Do you only work with investors, or can I buy a home to live in?
  "do-you-only-work-with-investors": "Работите ли само с инвеститори, или мога да купя дом, в който да живея?",

  // EN: Both, equally. Whether you're building a rental portfolio or relocating your family, the service is the same end-to-end process - sourcing, construction, design and furnishing. The only difference is what you do with the keys at the end.
  "both-equally-whether-you-re-building": "И двете - в равна степен. Независимо дали изграждате портфейл от наемни имоти, или преместате семейството си, услугата е един и същ цялостен процес - търсене, строителство, дизайн и обзавеждане. Единствената разлика е какво правите с ключовете накрая.",

  // EN: What is the typical rental yield?
  "what-is-the-typical-rental-yield": "Каква е типичната наемна доходност?",

  // EN: Gross rental yields typically range from 5–8% in major cities and 8–12% in tourism-driven coastal and ski markets.
  "gross-rental-yields-typically-range-from": "Брутната наемна доходност обикновено е 5–8% в големите градове и 8–12% на туристическите морски и ски пазари.",

  // EN: Do you manage the property after purchase?
  "do-you-manage-the-property-after": "Управлявате ли имота след покупката?",

  // EN: Yes - renovation, furnishing, tenant sourcing, rent collection, maintenance, and reporting. A single monthly statement.
  "yes-renovation-furnishing-tenant-sourcing-rent": "Да - ремонт, обзавеждане, намиране на наематели, събиране на наеми, поддръжка и отчетност. Един месечен отчет.",

  // EN: How long does a typical purchase take?
  "how-long-does-a-typical-purchase": "Колко време отнема типична покупка?",

  // EN: From accepted offer to keys typically takes 30–60 days including due diligence and notary formalities.
  "from-accepted-offer-to-keys-typically": "От приетата оферта до ключовете обикновено минават 30–60 дни, включително правната проверка и нотариалните формалности.",

  // EN: What are the total transaction costs?
  "what-are-the-total-transaction-costs": "Какви са общите разходи по сделката?",

  // EN: Typically 4–5% of the purchase price covering notary, municipal tax, registration, and legal fees.
  "typically-4-5-of-the-purchase": "Обикновено 4–5% от покупната цена, включващи нотариални такси, местен данък, вписване и правни услуги.",

  // EN: Notes on the Bulgarian market.
  "notes-on-the-bulgarian-market": "Бележки за българския пазар.",

  // EN: View all articles
  "view-all-articles": "Всички статии",

  // EN: Market
  "market": "Пазар",

  // EN: Guides
  "guides": "Ръководства",

  // EN: Modern furnished apartment interior delivered by Bulgaria Property Concierge
  "modern-furnished-apartment-interior-delivered-by": "Модерен обзаведен апартамент, предаден от Bulgaria Property Concierge",

  // EN: Ready to begin?
  "ready-to-begin": "Готови ли сте да започнем?",

  // EN: Book your investment consultation.
  "book-your-investment-consultation": "Запазете своята инвестиционна консултация.",

  // EN: A confidential, no-obligation conversation about your goals, timeline, and budget. We respond within 24 hours.
  "a-confidential-no-obligation-conversation-about": "Поверителен разговор без ангажимент за вашите цели, срокове и бюджет. Отговаряме в рамките на 24 часа.",

  // EN: One dedicated advisor, start to finish
  "one-dedicated-advisor-start-to-finish": "Един личен консултант от началото до края",

  // EN: Ten years of hands-on expertise
  "ten-years-of-hands-on-expertise": "Десет години практически опит",

  // EN: Legal, banking & development partners
  "legal-banking-and-development-partners": "Правни, банкови и строителни партньори",

  // EN: Move-in ready, completely hassle-free
  "move-in-ready-completely-hassle-free": "Готов за нанасяне, без никакви грижи",

  // EN: Get in Touch
  "get-in-touch": "Свържете се с нас",

  // EN: Request a private consultation.
  "request-a-private-consultation": "Заявете лична консултация.",

  // EN: Full name
  "full-name": "Име и фамилия",

  // EN: Your name
  "your-name": "Вашето име",

  // EN: Email
  "email": "Имейл",

  // EN: you@domain.com
  "you-domain-com": "you@domain.com",

  // EN: Phone number
  "phone-number": "Телефонен номер",

  // EN: Country code
  "country-code": "Код на държавата",

  // EN: Type of apartment
  "type-of-apartment": "Вид имот",

  // EN: Select one
  "select-one": "Изберете",

  // EN: 1-Bedroom Apartment
  "1-bedroom-apartment": "Апартамент с 1 спалня",

  // EN: 2-Bedroom Apartment
  "2-bedroom-apartment": "Апартамент с 2 спални",

  // EN: Multiple Bedrooms
  "multiple-bedrooms": "Няколко спални",

  // EN: House
  "house": "Къща",

  // EN: Investment goal
  "investment-goal": "Инвестиционна цел",

  // EN: Investment Property (Renting Out)
  "investment-property-renting-out": "Инвестиционен имот (за отдаване под наем)",

  // EN: Personal Residence
  "personal-residence": "Лично жилище",

  // EN: When do you plan to purchase
  "when-do-you-plan-to-purchase": "Кога планирате да купите",

  // EN: Within 1 Month
  "within-1-month": "До 1 месец",

  // EN: Up to 3 Months
  "up-to-3-months": "До 3 месеца",

  // EN: Up to 6 Months
  "up-to-6-months": "До 6 месеца",

  // EN: Just Exploring
  "just-exploring": "Само се оглеждам",

  // EN: Budget for property purchase
  "budget-for-property-purchase": "Бюджет за покупка на имот",

  // EN: Under €100k
  "under-100k": "Под 100 хил. €",

  // EN: €100k – €200k
  "100k-200k": "100 – 200 хил. €",

  // EN: €200k – €300k
  "200k-300k": "200 – 300 хил. €",

  // EN: €300k – €500k
  "300k-500k": "300 – 500 хил. €",

  // EN: €500k+
  "500k": "Над 500 хил. €",

  // EN: Type of furnished property
  "type-of-furnished-property": "Вид обзавеждане",

  // EN: Budget (Essential)
  "budget-essential": "Икономичен (Основен)",

  // EN: Mid-Range (Signature)
  "mid-range-signature": "Среден клас (Авторски)",

  // EN: Luxury (Bespoke)
  "luxury-bespoke": "Луксозен (Индивидуален)",

  // EN: Message <span class="text-stone-300 normal-case tracking-normal">(optional)</span>
  "message-optional": "Съобщение <span class=\"text-stone-300 normal-case tracking-normal\">(по желание)</span>",

  // EN: Tell us about your goals...
  "tell-us-about-your-goals": "Разкажете ни за вашите цели...",

  // EN: Request Consultation
  "request-consultation": "Заявете консултация",

  // EN: or message us directly
  "or-message-us-directly": "или ни пишете директно",

  // EN: Boutique investment and relocation for international clients in Bulgaria.
  "boutique-investment-and-relocation-for-international": "Бутикови инвестиции и релокация за международни клиенти в България.",

  // EN: Sofia, Bulgaria
  "sofia-bulgaria": "София, България",

  // EN: hello@bpc.bg
  "hello-bpc-bg": "hello@bpc.bg",

  // EN: Viber
  "viber": "Viber",

  // EN: Instagram
  "instagram": "Instagram",

  // EN: LinkedIn
  "linkedin": "LinkedIn",

  // EN: Residential
  "residential": "Жилищни",

  // EN: Commercial
  "commercial": "Търговски",

  // EN: Company
  "company": "Компания",

  // EN: About
  "about": "За нас",

  // EN: Contact
  "contact": "Контакти",

  // EN: Resources
  "resources": "Ресурси",

  // EN: Market Reports
  "market-reports": "Пазарни доклади",

  // EN: Investor Guide
  "investor-guide": "Ръководство за инвеститори",

  // EN: Relocation Guide
  "relocation-guide": "Ръководство за преместване",

  // EN: © 2026 Bulgaria Property Concierge. All rights reserved.
  "2026-bulgaria-property-concierge-all-rights": "© 2026 Bulgaria Property Concierge. Всички права запазени.",

  // EN: Privacy Policy
  "privacy-policy": "Политика за поверителност",

  // EN: Terms & Conditions
  "terms-and-conditions": "Общи условия",

  // EN: VIP Property Discovery Tour
  "vip-property-discovery-tour": "VIP тур за откриване на имоти",

  // EN: VIP Tour
  "vip-tour": "VIP тур",

  // EN: Chat on Viber: +359 88 234 5678
  "chat-on-viber-359-88-234": "Чат във Viber: +359 88 234 5678",

  // EN: Browse completed projects with real results, or properties currently available to purchase - investment, residential, or vacation, every one delivered to the same exacting standard.
  "browse-completed-projects-with-real-results": "Разгледайте завършени проекти с реални резултати или имоти, които в момента са налични за покупка - инвестиционни, жилищни или ваканционни, всеки от тях изпълнен по един и същ висок стандарт.",

  // EN: All Properties
  "all-properties": "Всички имоти",

  // EN: Completed Projects
  "completed-projects": "Завършени проекти",

  // EN: Available Listings
  "available-listings": "Налични обяви",

  // EN: City
  "city": "Град",

  // EN: All Cities
  "all-cities": "Всички градове",

  // EN: Sofia
  "sofia-2": "София",

  // EN: Plovdiv
  "plovdiv": "Пловдив",

  // EN: Varna
  "varna": "Варна",

  // EN: Bansko
  "bansko": "Банско",

  // EN: Sozopol
  "sozopol": "Созопол",

  // EN: Type
  "type": "Вид",

  // EN: All Types
  "all-types": "Всички видове",

  // EN: Vacation Rental
  "vacation-rental": "Ваканционен наем",

  // EN: New Build
  "new-build": "Ново строителство",

  // EN: Any
  "any": "Всякакви",

  // EN: 1 Bed
  "1-bed": "1 спалня",

  // EN: 2 Bed
  "2-bed": "2 спални",

  // EN: 3 Bed
  "3-bed": "3 спални",

  // EN: 4+ Bed
  "4-bed": "4+ спални",

  // EN: Apartment in Sofia Centre - Sofia · Centre · 2024
  "apartment-in-sofia-centre-sofia-centre": "Апартамент в центъра на София - София · Център · 2024",

  // EN: Sofia · Centre · 2024
  "sofia-centre-2024": "София · Център · 2024",

  // EN: Apartment in Sofia Centre
  "apartment-in-sofia-centre": "Апартамент в центъра на София",

  // EN: A compact city-centre apartment, finished and tenanted within weeks of handover.
  "a-compact-city-centre-apartment-finished": "Компактен апартамент в центъра на града, довършен и отдаден под наем броени седмици след предаването.",

  // EN: Price
  "price": "Цена",

  // EN: €135k
  "135k": "135 хил. €",

  // EN: Read Case Study
  "read-case-study": "Вижте проекта",

  // EN: Sea-View Loft in Varna - Varna · Sea Garden · 2024
  "sea-view-loft-in-varna-varna": "Лофт с морска панорама във Варна - Варна · Морска градина · 2024",

  // EN: Varna · Sea Garden · 2024
  "varna-sea-garden-2024": "Варна · Морска градина · 2024",

  // EN: Sea-View Loft in Varna
  "sea-view-loft-in-varna": "Лофт с морска панорама във Варна",

  // EN: A coastal loft with sweeping sea views, furnished for premium seasonal rental.
  "a-coastal-loft-with-sweeping-sea": "Крайбрежен лофт с широка морска панорама, обзаведен за премиум сезонен наем.",

  // EN: €110k
  "110k": "110 хил. €",

  // EN: 3.5 mo
  "3-5-mo": "3,5 мес.",

  // EN: Alpine Chalet in Bansko - Bansko · Alpine · 2023
  "alpine-chalet-in-bansko-bansko-alpine": "Алпийско шале в Банско - Банско · Алпийска зона · 2023",

  // EN: Bansko · Alpine · 2023
  "bansko-alpine-2023": "Банско · Алпийска зона · 2023",

  // EN: Alpine Chalet in Bansko
  "alpine-chalet-in-bansko": "Алпийско шале в Банско",

  // EN: Ski-in ski-out apartment, efficiently finished for peak-season rental income.
  "ski-in-ski-out-apartment-efficiently": "Апартамент с директен достъп до ски пистата, ефективно довършен за наемен доход в активния сезон.",

  // EN: €85k
  "85k": "85 хил. €",

  // EN: €165k
  "165k": "165 хил. €",

  // EN: €245k
  "245k": "245 хил. €",

  // EN: €320k
  "320k": "320 хил. €",

  // EN: Two-Bed New-Build for sale - Sofia · Lozenets
  "two-bed-new-build-for-sale": "Ново строителство с две спални, за продажба - София · Лозенец",

  // EN: For Sale
  "for-sale": "За продажба",

  // EN: Sofia · Lozenets
  "sofia-lozenets": "София · Лозенец",

  // EN: Two-Bed New-Build
  "two-bed-new-build": "Ново строителство с две спални",

  // EN: 2 Bed · 78 m²
  "2-bed-78-m": "2 спални · 78 кв.м",

  // EN: Under Construction · Delivery Q4 2026
  "under-construction-delivery-q4-2026": "В строеж · Предаване: IV тримесечие 2026",

  // EN: Enquire
  "enquire": "Запитване",

  // EN: Renovated City Apartment for sale - Sofia · Centre
  "renovated-city-apartment-for-sale-sofia": "Ремонтиран градски апартамент, за продажба - София · Център",

  // EN: Sofia · Centre
  "sofia-centre": "София · Център",

  // EN: Renovated City Apartment
  "renovated-city-apartment": "Ремонтиран градски апартамент",

  // EN: 1 Bed · 52 m²
  "1-bed-52-m": "1 спалня · 52 кв.м",

  // EN: Move-in Ready
  "move-in-ready": "Готов за нанасяне",

  // EN: Sea-View Apartment for sale - Varna · Sea Garden
  "sea-view-apartment-for-sale-varna": "Апартамент с морска панорама, за продажба - Варна · Морска градина",

  // EN: Varna · Sea Garden
  "varna-sea-garden": "Варна · Морска градина",

  // EN: Sea-View Apartment
  "sea-view-apartment": "Апартамент с морска панорама",

  // EN: 2 Bed · 68 m²
  "2-bed-68-m": "2 спални · 68 кв.м",

  // EN: Ski-In Ski-Out Chalet for sale - Bansko · Alpine
  "ski-in-ski-out-chalet-for": "Шале до ски пистата, за продажба - Банско · Алпийска зона",

  // EN: Bansko · Alpine
  "bansko-alpine": "Банско · Алпийска зона",

  // EN: Ski-In Ski-Out Chalet
  "ski-in-ski-out-chalet": "Шале до ски пистата",

  // EN: 1 Bed · 45 m²
  "1-bed-45-m": "1 спалня · 45 кв.м",

  // EN: Old Town Family Townhouse for sale - Plovdiv · Old Town
  "old-town-family-townhouse-for-sale": "Семейна къща в Стария град, за продажба - Пловдив · Старият град",

  // EN: Plovdiv · Old Town
  "plovdiv-old-town": "Пловдив · Старият град",

  // EN: Old Town Family Townhouse
  "old-town-family-townhouse": "Семейна къща в Стария град",

  // EN: 3 Bed · 140 m²
  "3-bed-140-m": "3 спални · 140 кв.м",

  // EN: Coastal Villa, Off-Plan for sale - Sozopol · Coastal
  "coastal-villa-off-plan-for-sale": "Крайбрежна вила на зелено, за продажба - Созопол · Крайбрежие",

  // EN: Sozopol · Coastal
  "sozopol-coastal": "Созопол · Крайбрежие",

  // EN: Coastal Villa, Off-Plan
  "coastal-villa-off-plan": "Крайбрежна вила на зелено",

  // EN: 4 Bed · 220 m²
  "4-bed-220-m": "4 спални · 220 кв.м",

  // EN: Off-Plan · Delivery Q2 2027
  "off-plan-delivery-q2-2027": "На зелено · Предаване: II тримесечие 2027",

  // EN: Boutique Office Space for sale - Sofia · Lozenets
  "boutique-office-space-for-sale-sofia": "Бутиково офис пространство, за продажба - София · Лозенец",

  // EN: Boutique Office Space
  "boutique-office-space": "Бутиково офис пространство",

  // EN: Office · 180 m²
  "office-180-m": "Офис · 180 кв.м",

  // EN: New-Build Penthouse for sale - Sofia · Ivan Vazov
  "new-build-penthouse-for-sale-sofia": "Пентхаус в нова сграда, за продажба - София · Иван Вазов",

  // EN: Sofia · Ivan Vazov
  "sofia-ivan-vazov": "София · Иван Вазов",

  // EN: New-Build Penthouse
  "new-build-penthouse": "Пентхаус в нова сграда",

  // EN: 3 Bed · 130 m²
  "3-bed-130-m": "3 спални · 130 кв.м",

  // EN: Under Construction · Delivery Q1 2027
  "under-construction-delivery-q1-2027": "В строеж · Предаване: I тримесечие 2027",

  // EN: Mountain-View Apartment for sale - Bansko · Alpine
  "mountain-view-apartment-for-sale-bansko": "Апартамент с планинска панорама, за продажба - Банско · Алпийска зона",

  // EN: Mountain-View Apartment
  "mountain-view-apartment": "Апартамент с планинска панорама",

  // EN: 1 Bed · 40 m²
  "1-bed-40-m": "1 спалня · 40 кв.м",

  // EN: No properties match this filter. <a href="index.html#contact" class="underline text-gold-dark">Tell us what you need</a> and we'll source it.
  "no-properties-match-this-filter-tell": "Няма имоти, които отговарят на този филтър. <a href=\"index.html#contact\" class=\"underline text-gold-dark\">Кажете ни какво търсите</a> и ние ще го намерим.",

  // EN: Don't see the right fit?
  "don-t-see-the-right-fit": "Не намирате подходящото?",

  // EN: Most of what we source never makes it to a public listing.
  "most-of-what-we-source-never": "По-голямата част от това, което намираме, никога не стига до публична обява.",

  // EN: Tell us your budget and goals - we'll go find it, on-market or off.
  "tell-us-your-budget-and-goals": "Кажете ни бюджета и целите си - ще го намерим, на пазара или извън него.",

  // EN: Tell Us What You Need
  "tell-us-what-you-need": "Кажете ни какво търсите",

  // EN: Sending…
  "form-sending": "Изпращане…",

  // EN: Thank you - we've received your request and will be in touch within 24 hours.
  "form-success": "Благодарим ви - получихме заявката ви и ще се свържем с вас в рамките на 24 часа.",

  // EN: I'm interested in the VIP Property Discovery Tour (3-day private tour, €300 - credited toward my purchase).
  "vip-prefill-message": "Интересувам се от VIP тура за откриване на имоти (3-дневен частен тур, €300 - приспада се изцяло от цената на покупката).",

  // EN: Too many submissions. Please try again later.
  "form-error-rate": "Твърде много опити за изпращане. Моля, опитайте отново по-късно.",

  // EN: Something went wrong. Please try again.
  "form-error-retry": "Нещо се обърка. Моля, опитайте отново.",

  // EN: Could not send your request. Please try again or message us directly.
  "form-error-send": "Заявката не можа да бъде изпратена. Моля, опитайте отново или ни пишете директно.",

  // EN: {n} property
  "count-one": "{n} имот",

  // EN: {n} properties
  "count-many": "{n} имота",

  // EN: Do you handle turnkey renovation, or do I need to hire contractors myself?
  "do-you-handle-turnkey-renovation": "Правите ли ремонт до ключ, или трябва сам да наема изпълнители?",

  // EN: Turnkey is the whole point. We manage architects, licensed contractors and site supervision from the first wall opened to the final photo - you approve decisions, we run the work. No separate contractors to hire or manage.
  "turnkey-is-the-whole-point-we-manage": "Точно затова предлагаме ремонт до ключ. Управляваме архитекти, лицензирани изпълнители и надзор на обекта - от първата съборена стена до финалната снимка. Вие одобрявате решенията, ние вършим работата. Не се налага сами да наемате и координирате изпълнители.",

  // EN: What's the difference between the Essential, Signature and Bespoke finish levels?
  "whats-the-difference-between-essential-signature": "Каква е разликата между нивата Основен, Авторски и Индивидуален?",

  // EN: Essential is an efficient, rental-ready finish for buy-to-let budgets. Signature adds premium materials and a designed interior throughout. Bespoke is fully custom architecture, joinery and furniture for owner-occupiers or luxury lets. The process is identical - only the finish level and cost change.
  "essential-is-an-efficient-rental-ready": "Основен е ефективно изпълнение, готово за отдаване под наем, за инвестиционни бюджети. Авторски добавя висококачествени материали и цялостен интериорен дизайн. Индивидуален е напълно персонализирана архитектура, дограма и мебели за собственици или луксозен наем. Процесът е един и същ - само нивото на изпълнение и цената се променят.",

  // EN: Do you offer interior design and furnishing, or only construction?
  "do-you-offer-interior-design-and-furnishing": "Предлагате ли интериорен дизайн и обзавеждане, или само строителство?",

  // EN: Both, as one service. Every renovation includes interior design - layout, materials, lighting and a bespoke furniture package - so the property is delivered styled and ready to list or move into, not just built.
  "both-as-one-service-every-renovation": "И двете - като една услуга. Всеки ремонт включва интериорен дизайн - оформление, материали, осветление и индивидуален пакет мебели - така че имотът се предава стилно оформен и готов за отдаване под наем или нанасяне, а не просто построен.",

  // EN: Can you source premium or luxury properties, not just renovate them?
  "can-you-source-premium-or-luxury-properties": "Можете ли да намирате премиум или луксозни имоти, а не само да ги ремонтирате?",

  // EN: Yes. Property sourcing is the first step of the service, and includes premium and luxury listings - as well as off-market opportunities - in Sofia, Plovdiv, Varna, Bansko and Sozopol, matched to your budget and investment goals.
  "yes-property-sourcing-is-the-first-step": "Да. Търсенето на имот е първата стъпка от услугата и включва премиум и луксозни обяви - както и извънпазарни възможности - в София, Пловдив, Варна, Банско и Созопол, съобразени с вашия бюджет и инвестиционни цели.",

  // EN: How much does a full renovation and furnishing package cost?
  "how-much-does-a-full-renovation-and-furnishing": "Колко струва пълен пакет ремонт и обзавеждане?",

  // EN: Renovation and furnishing budgets typically range from Essential buy-to-let finishes to fully bespoke luxury execution, scoped to the property and finish level after a walkthrough. Every quote is itemized before work begins, with no hidden costs.
  "renovation-and-furnishing-budgets-typically": "Бюджетите за ремонт и обзавеждане обикновено варират от изпълнение Основен за инвестиционни имоти до напълно индивидуално луксозно изпълнение, определени спрямо имота и нивото на изпълнение след оглед. Всяка оферта е детайлна преди началото на работата, без скрити разходи.",

  // EN: Do I need to be in Bulgaria during the renovation or design process?
  "do-i-need-to-be-in-bulgaria-during-renovation": "Трябва ли да съм в България по време на ремонта или дизайна?",

  // EN: No. Most clients manage the entire process - design approvals, material selections and progress updates - remotely, with photo and video updates at each milestone. Many never visit the property until it's finished.
  "no-most-clients-manage-the-entire": "Не. Повечето клиенти управляват целия процес - одобрения на дизайна, избор на материали и напредъка - дистанционно, с актуализации със снимки и видео при всеки етап. Много клиенти виждат имота за първи път едва след завършването му.",

  // EN: A Sofia house covered in autumn ivy among the rooftops of a residential district at sunset
  "sofia-rental-yields-by-neighbourhood-2026-photo": "Къща в София, обвита в есенен бръшлян, сред покривите на жилищен квартал по залез",

  // EN: Sofia at sunset: domes, historic buildings and a tree-lined street
  "buying-property-in-bulgaria-as-a-foreigner-photo": "София по залез: куполи, исторически сгради и улица със зеленина",

  // EN: Two professionals reviewing and signing a property document at a table
  "property-due-diligence-bulgaria-photo": "Двама професионалисти преглеждат и подписват документ за имот на маса",

  // EN: 25 September 2026
  "25-september-2026": "25 септември 2026",

  // EN: Sofia rental yields by neighbourhood: the full 2026 map.
  "sofia-rental-yields-by-neighbourhood-the": "Наемна доходност в София по квартали: пълна карта за 2026.",

  // EN: The national average hides a gap of more than 2 percentage points between Sofia's strongest and weakest districts.
  "the-national-average-hides-a-gap": "Средното число за страната крие разлика от над 2 процентни пункта между най-силните и най-слабите квартали на София.",

  // EN: How a foreigner buys property in Bulgaria: 7 steps, no surprises.
  "how-a-foreigner-buys-property-in": "Как чужденец купува имот в България: 7 стъпки без изненади.",

  // EN: Your rights depend on your citizenship, and the land under a house is treated differently from the building itself.
  "your-rights-depend-on-your-citizenship": "Правата зависят от гражданството, а земята под къщата се третира различно от самата сграда.",

  // EN: The due diligence that saves the deal.
  "the-due-diligence-that-saves-the": "Правната проверка, която спасява сделката.",

  // EN: The notary checks the registered owner and registered encumbrances. That's it. Here's what falls outside that check.
  "the-notary-checks-the-registered-owner": "Нотариусът проверява вписания собственик и регистрираните тежести. Това е всичко. Ето какво остава извън проверката.",

  // EN: Journal | Guides & Market Insights on Bulgarian Property | Bulgaria Property Concierge
  "meta-title-blog": "Блог | Ръководства и пазарни анализи за имоти в България | Bulgaria Property Concierge",

  // EN: Practical guides and market data for buying, renovating and renting out property in Bulgaria: rental yields in Sofia, how foreigners buy, and legal due diligence.
  "meta-description-blog": "Практични ръководства и пазарни данни за покупка, ремонт и отдаване под наем на имот в България: доходност в София, как купуват чужденци и правна проверка.",

  // EN: Practical guides and market data for buying, renovating and renting out property in Bulgaria.
  "practical-guides-and-market-data-for": "Практични ръководства и пазарни данни за покупка, ремонт и отдаване под наем на имот в България.",

  // EN: Want to talk it through?
  "want-to-talk-it-through": "Искате да го обсъдим?",

  // EN: Tell us what you are looking for and we will show you what it takes, step by step.
  "tell-us-what-you-are-looking": "Кажете ни какво търсите и ще ви покажем какво е нужно, стъпка по стъпка.",

  // EN: Book a free consultation
  "book-a-free-consultation-2": "Запазете безплатна консултация",

};
